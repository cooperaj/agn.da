...
static void window_load(Window *window) {
  ...
  text_layer_set_text(s_title_layer, DIALOG_SETUP_WINDOW_APP_NAME);
  ...
  GSize title_height = text_layer_get_content_size(s_title_layer);
  const GEdgeInsets icon_insets = {
    .top = title_height.h + 15,
    .right = (bounds.size.w - bitmap_bounds.size.w) / 2,
    .bottom = bounds.size.h - bitmap_bounds.size.h - (title_height.h + 15),
  ...
  const GEdgeInsets body_insets = {.top = 120, .right = 5, .left = 5};
  s_body_layer = text_layer_create(grect_inset(bounds, body_insets));
  text_layer_set_text(s_body_layer, DIALOG_SETUP_WINDOW_MESSAGE);
